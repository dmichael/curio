...
AR.IO Core has no Curio pin API. Storing a reference fully reads the requested
...
async def store_arweave(txid: str, path: str, settings: Settings, client: httpx.AsyncClient) -> str:
    ...
    try:
        ...
        return "failed"
    return "stored" if headers.get("x-cache", "").strip().lower() == "hit" else "failed"
