...
it does not faithfully exercise XDG-root installation, bind-mount ownership,
host port publication, Docker restart policies, or persistence across a guest
reboot. The detailed 0.1 `/opt`, `/etc`, and `/var/lib` examples below remain
useful qualification rationale, but current runs use the installing user's
`$XDG_CONFIG_HOME/curio` and `$XDG_DATA_HOME/curio/{app,state}` roots.

The current `dev/test-appliance.sh` additionally records the immutable release
pointer, AR.IO first-install height, a static-media fixture, state ownership,
update/rerun behavior, deliberate dependency failure recovery, and the
published-port set. Its `--after-reboot` phase is evidence only when it is
actually run in the same disposable VM after a real reboot.
